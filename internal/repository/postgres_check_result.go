...
func (r *PostgresCheckResultRepository) GetBySiteID(siteID string, limit, offset int) ([]domain.CheckResult, error) {
	query := `SELECT id, site_id, status, response_time_ms, checked_at FROM check_results WHERE site_id = $1 ORDER BY checked_at DESC LIMIT $2 OFFSET $3;`
...
func (r *PostgresCheckResultRepository) GetLatestBySiteID(siteID string) (*domain.CheckResult, error) {
	query := `SELECT id, site_id, status, response_time_ms, checked_at FROM check_results WHERE site_id = $1 ORDER BY checked_at DESC LIMIT 1;`
	var rsl domain.CheckResult
